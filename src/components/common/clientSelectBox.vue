<template>
  ...
  >
    ...
    <template v-for="itemone in items" :key="itemone.client_no">
      <option :value="itemone.client_no">
        {{ itemone.client_name }}
      </option>
    </template>
  </select>
...
export default {
  ...
  mounted: function () {
    ...
    this.axios
      .post('/accounting/clientlist.do', params)
      .then(function (response) {
        console.log(response);

        vm.items = response.data.clientlist;
